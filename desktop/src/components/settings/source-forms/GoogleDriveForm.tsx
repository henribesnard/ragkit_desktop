...
export function GoogleDriveForm({ config, onChange, sourceId, connected, onConnected }: GoogleDriveFormProps) {
    ...
    return (
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Connexion Google</label>
                <OAuthButton provider="google" sourceId={sourceId} connected={connected} onConnected={onConnected} />
                {!sourceId && (
            ...
            <CloudFolderPicker
                label="IDs des dossiers Google Drive"
                value={config.folder_ids || []}
                onChange={(value) => onChange({ ...config, folder_ids: value })}
                disabled={!connected}
                placeholder="1ABCDEF..."
            />
            ...
            <div className="flex items-center gap-2">
                <input
                    type="checkbox"
                    id="include-shared"
                    checked={config.include_shared === true}
                    onChange={(e) => onChange({ ...config, include_shared: e.target.checked })}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <label htmlFor="include-shared" className="text-sm text-gray-700 dark:text-gray-300">Inclure les fichiers partages</label>
            </div>

            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Types MIME autorises</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-2 mt-6">
                    <input
                        ...
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <label htmlFor="gd-recursive" className="text-sm text-gray-700 dark:text-gray-300">Scanner les sous-dossiers</label>
                </div>
                <div>
