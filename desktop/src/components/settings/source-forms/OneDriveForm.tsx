...
export function OneDriveForm({ config, onChange, sourceId, connected, onConnected }: OneDriveFormProps) {
    ...
    return (
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Connexion Microsoft</label>
                <OAuthButton provider="microsoft" sourceId={sourceId} connected={connected} onConnected={onConnected} />
                {!sourceId && (
            ...
            <CloudFolderPicker
                label="Chemins de dossiers OneDrive"
                value={config.folder_paths || []}
                onChange={(value) => onChange({ ...config, folder_paths: value })}
                disabled={!connected}
                placeholder="/Documents/Projet"
            />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Drive ID (optionnel)</label>
                    <input
                        type="text"
                        value={config.drive_id || ""}
                        onChange={(e) => onChange({ ...config, drive_id: e.target.value || null })}
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Site ID SharePoint (optionnel)</label>
                    <input
                        type="text"
                        value={config.site_id || ""}
                        onChange={(e) => onChange({ ...config, site_id: e.target.value || null })}
                        className="block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center gap-2 mt-6">
                    ...
                    />
                    <label htmlFor="od-recursive" className="text-sm text-gray-700 dark:text-gray-300">Scanner les sous-dossiers</label>
                </div>
                <div>
